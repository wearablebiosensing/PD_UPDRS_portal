...
@callback(
    ...
    Input('task_id','value'),
    Input('activity_id','value'))
def update_graph2(pid, dates_id, task_id,activity_id):
    # Query for specfic patients.
    file_path = file_path_root + str(pid) + "/" + str(dates_id) + "/"+ str(task_id)
    ...
    exercise_name = activity_codes("lg",activity_id)
    df_dates_pid_p = df_dates_pid[df_dates_pid["ParticipantList"]==pid]
    normalized_arr = preprocessing.normalize([df_lg_activity["index"][25:500]])
    ...
    valley_idx, properties_idx = find_peaks(normalized_arr.ravel()*-1, width=(2,20),distance=5)     #  prominence=(0.01, 4)  
    ...
    # # for P3 - width=4
    indices = find_peaks(normalized_arr.ravel(), height=quantiles_normalized_arr)[0] 
    ...
    valley, properties = find_peaks(index_minus_thumb*-1, prominence=(0.01, None), width=(2,20),distance=5)        

    #index_minus_thumb_df = pd.DataFrame(index_minus_thumb, columns =['idx_minus_thumb'])
    print("Difference values: ")
    print(index_minus_thumb.diff().ravel(),type(index_minus_thumb.diff()))
    
    # print("DOT PRODUCT INDEX AND THUMB",np.dot(df_lg_activity["index"][:590],df_lg_activity["thumb"][:590])
    # Vertical spacing cannot be greater than (1 / (rows - 1)) = 0.142857.
    fig2 = make_subplots(rows=5, cols=1, vertical_spacing = 0.13,subplot_titles=("Index, Number of Peaks- " +str(len(indices)),"Middle","Thumb","Index-Thumb, Number of Peaks- " +str(len(indices_idx_thumb)),"Index-Thumb Difference / Slope of the cosective points"))
    ...
    fig2.update_yaxes(title_text="Voltage", title_font_family="IBM Plex San", row=3, col=1)
